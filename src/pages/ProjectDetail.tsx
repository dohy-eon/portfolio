...
const ProjectDetailPage = () => {
  ...
  const SectionTitle = ({ number, title }: { number: string; title: string }) => (
    <div className="border-t-2 border-zinc-600 pt-6 mt-12 mb-8">
      <div className="flex items-baseline gap-4">
        <span className="text-zinc-400 font-mono text-sm md:text-base font-bold tracking-wider">{number}</span>
        <h3 className="text-xl md:text-2xl lg:text-3xl font-galmuri font-bold text-white">{title}</h3>
      </div>
    </div>
  );
  ...
  const getProjectImage = () => {
    ...
    const imageMap: Record<number, string> = {
      7: valuediBgImage.startsWith('http') ? valuediBgImage : `${siteUrl}${valuediBgImage}`,
          ...
          <div className="px-4 md:px-8 lg:px-12 xl:px-20 py-8 md:py-12 lg:py-16 max-w-7xl mx-auto">
            ...
            <div className="mb-12">
              ...
              <div className="flex flex-wrap gap-2 md:gap-3 mb-4">
                {project.stack.map((tech) => (
                  <span
                    key={tech}
                    className="px-4 py-2 bg-zinc-800/50 rounded-full text-sm md:text-base text-zinc-300 border border-zinc-700 font-mono"
                  >
                    {tech}
                  </span>
                ))}
              </div>
              {'tags' in project && Array.isArray((project as any).tags) && (project as any).tags.length > 0 && (
                <div className="flex flex-wrap gap-2 md:gap-3 mt-2">
                  {(project as any).tags.map((tag: string) => (
                    <span
                      key={tag}
                      className="px-3 py-1.5 rounded-full text-xs md:text-sm bg-violet-500/10 text-violet-300 border border-violet-500/40 font-mono tracking-tight"
                    >
                      {tag}
                    </span>
                  ))}
            ...
            {project.details && (
              <div className="space-y-12">
                  ...
                  <ul className="grid md:grid-cols-3 gap-4 lg:gap-6">
                    {project.details.features.map((feature, idx) => {
                      ...
                      return (
                        <li
                          key={idx}
                          className="text-sm md:text-base border border-zinc-600 p-5 lg:p-6 bg-zinc-800/30 rounded-lg hover:bg-zinc-800/50 transition-colors"
                        >
                          {hasKeyword ? (
                            <>
                ...
                {project.details.leadership && (
                  <div>
                    <SectionTitle number="04" title="리더십 & 협업" />
                ...
                {/* Major PRs */}
                {project.details.majorPRs && project.details.majorPRs.length > 0 && (
                  <div>
                    <SectionTitle number={project.details.leadership ? '05' : '04'} title="Major PRs" />
                    <ul className="space-y-3 md:space-y-4">
                      {(project.details.majorPRs as MajorPR[]).map((pr, idx) => (
                        <li
                          key={`${pr.number}-${idx}`}
                          className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 px-4 py-3 bg-zinc-900/60 border border-zinc-700 rounded-lg"
                        >
                          <div>
                            <p className="text-xs md:text-sm text-violet-300 font-mono">PR #{pr.number}</p>
                            <p className="text-sm md:text-base text-gray-200 mt-1">{pr.title}</p>
                          </div>
                          {pr.url && (
                            <a
                              href={pr.url}
                              ...
                              className="inline-flex items-center justify-center px-3 py-1.5 text-xs md:text-sm rounded-md border border-violet-400 text-violet-200 hover:bg-violet-500/10 transition-colors"
                            >
                              GitHub PR 보기
                            </a>
                          )}
                ...
                {/* Code Snippets */}
                {project.details.codeSnippets && project.details.codeSnippets.length > 0 && (
                  <div>
                    <SectionTitle
                      number={
                        project.details.leadership
                    ...
                    <div className="space-y-6">
                      {project.details.codeSnippets.map((snippet, idx) => (
                        <div key={idx} className="border border-zinc-700 rounded-xl bg-zinc-900/70 overflow-hidden">
                          <div className="px-4 md:px-6 py-3 border-b border-zinc-700 bg-zinc-900/80 flex items-center justify-between">
                            <div>
                              <h4 className="text-sm md:text-base font-semibold text-zinc-100">{snippet.title}</h4>
                              <p className="text-xs md:text-sm text-zinc-400 mt-1">{snippet.description}</p>
                            </div>
                          </div>
                          <div className="p-4 md:p-6 bg-black/60 overflow-x-auto">
                            <pre className="text-xs md:text-sm font-mono text-zinc-100 whitespace-pre">
                              {snippet.code}
                            </pre>
                          </div>
                        </div>
