...
export const PROFILE = {
  ...
  bio: [
    ...
    "사용자 경험을 중심에 두고, 명확한 흐름·완성도·퍼포먼스를 우선순위에 두는 프론트엔드 개발자입니다.",
    "React, TypeScript, Tailwind CSS를 기반으로 직관적인 UI와 구조적인 설계를 좋아하고 서비스의 동작 원리를 끝까지 파고드는 편이에요.",
    "프로젝트에서는 PM 또는 TL로 주도적으로 흐름을 만들며 기능 정의 → UX 설계 → 개발 → 배포까지 전체 프로덕트 사이클을 이해하고 설명할 수 있도록 노력합니다. AI, Web3, XR, 커뮤니티 서비스 등 다양한 도메인 경험을 쌓아왔고, 기술 중심보다 문제를 어떻게 풀어낼지에 더 집중하면서 서비스를 개발하는 편이에요."
  ]
};
...
export const PROJECTS = [
  {
    ...
    details: {
      ...
      techTopics: [
        ...
        {
          title: "Recoil을 이용한 효율적인 전역 상태 관리",
          desc: "사용자 정보, 온보딩 데이터, 언어 설정 등 전역적으로 관리해야 할 상태를 Recoil Atom으로 정의하여 관리했습니다. 온보딩 과정에서 단계별 데이터 유실을 방지하고, 컴포넌트 간 불필요한 Props Drilling을 제거하여 코드의 유지보수성을 향상시켰습니다."
        }
      ],
  ...
  },
  {
    ...
    details: {
      ...
      techTopics: [
        {
          ...
          desc: "ProjectGenerator, ConfigManager 등 역할에 따라 클래스를 분리하여 유지보수성을 높였습니다. 특히 상태 관리 라이브러리별 설정(Config)을 모듈화하여, 추후 새로운 라이브러리 지원이 필요할 때 기존 코드를 최소한으로 수정하며 확장할 수 있는 구조를 구축했습니다."
        },
        {
      ...
      retrospective: "최종 사용자가 아닌 '개발자'를 위한 도구를 만들며 DX(Developer Experience)에 대해 깊이 고민할 수 있었습니다. 특히 파일 시스템(fs)을 직접 다루며 프로젝트 구조를 동적으로 생성하는 로직과, 배포 자동화 스크립트를 작성하는 과정에서 Node.js 환경과 소프트웨어 배포 생명주기에 대한 이해도를 크게 높였습니다."
    }
  },
  {
    id: 1,
