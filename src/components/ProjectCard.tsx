    ...
    <>
      ...
      >
        ...
        >
          ...
          {/* 하단 텍스트 영역 */}
          <div
            className={`flex-1 flex flex-col justify-between ${isGridLayout ? 'p-5 md:p-6' : 'p-3 md:p-4 bg-white'}`}
          >
            <div className="flex-1">
              {/* 제목과 카테고리 */}
              <div className={`flex items-start justify-between mb-3 ${isGridLayout ? 'mb-4' : 'mb-1.5'}`}>
                <h3
                  className={`font-bold ${
                    isGridLayout ? 'text-lg md:text-xl text-white font-galmuri' : 'text-base md:text-lg text-black'
                  }`}
                >
                  {project.title}
                </h3>
            ...
            <div
              className={`flex items-center justify-between ${
                isGridLayout ? 'pt-4 border-t border-zinc-700/50' : 'pt-2 border-t border-zinc-100'
              }`}
            >
              ...
              {!isGridLayout && (
                <span className="text-xs text-zinc-400 font-medium">{project.role.split('&')[0].trim()}</span>
              )}
            </div>
          </div>
        </div>
